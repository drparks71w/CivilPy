...
def _parse_size_label(label):
    """Parse a size label like "6x12", "6X12", or "6 x 12" into (width, depth).

    Returns numbers as int when whole, float otherwise (e.g. "6.75x30").

    >>> _parse_size_label("6x12")
    (6, 12)
    >>> _parse_size_label("6.75 X 30")
    (6.75, 30)
    """
    parts = label.lower().replace(" ", "").split("x")
    if len(parts) != 2:
        raise ValueError(
            f"Could not parse size label '{label}'. "
            f"Expected the form 'WIDTHxDEPTH', e.g. '6x12'."
        )
    dims = []
    for p in parts:
        v = float(p)
        dims.append(int(v) if v.is_integer() else v)
    return tuple(dims)
...
class LumberSection:
    ...
    Can be constructed either from a size label (like steel sections,
    e.g. ``W("W36X150")``) or from separate width/depth values:

        >>> beam = LumberSection("6x12", "Douglas Fir-Larch", "No. 1")
        >>> beam = LumberSection(6, 12, "Douglas Fir-Larch", "No. 1")

    Args:
        nominal_width: Nominal width in inches (e.g. 2, 4, 6, 8), or a
            size label string such as "6x12" (in which case the remaining
            arguments shift left: ``LumberSection("6x12", species, grade)``).
    ...
    def __init__(self, nominal_width, nominal_depth, species=None, grade=None):
        if isinstance(nominal_width, str):
            # Label form: LumberSection("6x12", species, grade)
            if grade is not None:
                raise TypeError(
                    "When using a size label, pass only species and grade: "
                    "LumberSection('6x12', species, grade)"
                )
            species, grade = nominal_depth, species
            nominal_width, nominal_depth = _parse_size_label(nominal_width)
        if species is None or grade is None:
            raise TypeError("species and grade are required")
    ...
    def _determine_size_class(self):
        ...
        # Dimension lumber -- reference values are tabulated in grouped
        # depth classes (2x4, 2x6-2x8, 2x10-2x12); CF uses the individual
        # size via _cf_size_class()
        if nd == 4:
            return "2x4"
        if nd <= 8:
            return "2x6-2x8"
...
class GlulamSection:
    ...
    Args:
        width: Actual width in inches (e.g. 5.125, 6.75, 8.75, 10.75),
            or a size label string such as "6.75x30" (in which case the
    ...
    def __init__(self, width, depth, combination=None, species=None):
        if isinstance(width, str):
            # Label form: GlulamSection("6.75x30", combination, species)
            if species is not None:
                raise TypeError(
                    "When using a size label, pass only combination and "
                    "species: GlulamSection('6.75x30', combination, species)"
                )
            combination, species = depth, combination
            width, depth = _parse_size_label(width)
        if combination is None or species is None:
            raise TypeError("combination and species are required")
