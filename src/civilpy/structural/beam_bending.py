...
class Beam:
    ...
    * Units: lengths in ft, forces in kips, moments in kip·ft.
    ...
    def add_loads(self, loads: list):
        ...
        Args:
            loads (list): An iterable containing DistributedLoad or PointLoad objects to
    ...
    def get_reaction_forces(self):
        ...
        Returns:
            tuple: (F_Ax, F_Ay, F_By) reaction force components for pinned (x,y) and rolling (y) supports respectively.
        """
    ...
    def plot(self):
        ...
        Returns:
            matplotlib.figure.Figure: figure with the 4 subplots.
        """
    ...
    def _create_distributed_force(self, load: DistributedLoadH or DistributedLoadV, shift: bool = True):
        ...
        Args:
            load (DistributedLoadH or DistributedLoadV): The distributed load object.
            shift (bool): when set to False, the x-coordinate in the expression is referred to the left end of the beam,
                instead of the left end of the provided interval. Defaults to True.

        Returns:
            sympy.Piecewise: object with the value of the distributed load.
        """
        expr, interval = load
        x0, x1 = interval
        expr = sympify(expr)
    ...
    def _effort_from_pointload(self, load: PointLoadH or PointLoadV):
        """Create a sympy.Piecewise object representing the shear force caused by a point load.

        Args:
            load (PointLoadH or PointLoadV): The point load object containing value and coordinate.

        Returns:
            sympy.Piecewise: object with the value of the shear force produced by the provided point load.
        """
        value, coord = load
...
class CantileverBeam(Beam):
    ...
    def get_reaction_forces(self):
        ...
        Returns:
            tuple: (V_A, M_A) Vertical reaction force (positive upward) and moment (positive counter-clockwise)
