...
def _create_brr_engine(connection_string: Optional[str] = None):
    ...
    host = secrets.get("BRR_SERVER", "localhost")
    port = secrets.get("BRR_PORT", "1521")
    service = secrets.get("BRR_SERVICE", "BRR")
    user = secrets.get("BRR_USN", "")
    password = secrets.get("BRR_PASS", "")
...
# //TODO - AI REVIEW & DEBUGGING SUMMARY - ODOT BrR DATABASE INTEGRATION
#
# #### 1. DATABASE SCHEMA MISMATCH (Rating Filter)
# * **Original Error:** The script was hardcoded to filter by `UP_TO_DATE_IND = 'Y'`.
# * **Discovery:** Metadata search revealed the ODOT AASHTOWare BrR database uses 'T' (True) and 'F' (False) instead of 'Y'/'N'.
# * **Fix:** Updated all SQL queries in `_fetch_controlling_rating`, `_fetch_member_capacities`, and `_fetch_vehicle_loads` to use `UP_TO_DATE_IND = 'T'`.
#
# #### 2. DATA EXISTENCE (SFN Selection)
# * **Original Error:** SFN '2508419' was returning `None` and `0`.
# * **Discovery:** Mathematical verification via COUNT(*) queries proved that while the bridge existed in the `ABW_BRIDGE` table, no analysis events existed in the `ABW_SPNG_MBR_ALT_EVENTS` table.
# * **Fix:** Migrated testing to SFN '6006183' which contains verified active analysis records.
#
# #### 3. HIERARCHICAL MAPPING (Structural Tables)
# * **Original Error:** Attempted to pull span lengths from `ABW_SUPER_STRUCT_SPAN` (non-existent) and beam names using assumed column names like `MEMBER_ID`.
# * **Discovery:** DDL review showed bridge geometry is split across `ABW_SUPER_STRUCT_MBR` (names), `ABW_SUPER_STRUCT_MBR_SPAN` (lengths), and `ABW_SUPER_STRUCT_SPNG_MBR_ALT` (definitions).
# * **Fix:** Rewrote `_fetch_structural_data` to map `SUPER_STRUCT_MBR_ID` across these specific tables.
#
# #### 4. POLYMORPHIC JOIN ERRORS (Design Info)
# * **Original Error:** Multiple `ORA-00904` (Invalid Identifier) errors. Hallucinated join columns `rolled.STL_SHAPE_ID`, `ishape.ISHAPE_ID`, and `plate.WEB_DEPTH`.
# * **Discovery:** Metadata/Column searches proved `ABW_STL_ROLLED_IBEAM_DEF` has no shape-specific IDs; it only holds the foreign key `SPNG_MBR_DEF_ID`.
# * **Fix:** Rewrote `fetch_design_info` using a `COALESCE` strategy. Joined `ABW_SUPER_STRUCT_SPNG_MBR_ALT` directly to `ABW_STL_ISHAPE` (via `STL_SHAPE_ID`) and `ABW_STL_XSECTION` (via `SPNG_MBR_DEF_ID`).
#
# #### 5. PYTHON DATA HANDLING (NoneType Formatting)
# * **Original Error:** `TypeError: unsupported format string passed to NoneType.__format__`.
# * **Discovery:** Query successfully returned rows, but some structural members (like Plate Girders vs. Rolled Shapes) returned `NULL` for specific columns.
# * **Fix:** Updated `display_design_summary` to use conditional expressions (ternary operators) to check for `None` before applying float formatting (`:.2f`).
#
# ### CURRENT STATUS
# * The script successfully pulls NBI data, active Load Ratings, Structural Hierarchy, and Physical Steel Dimensions for SFN '6006183'.
# * The bridge type was identified as a Floorbeam-Stringer system with a mix of Rolled and Plate Girder components.
...
class BrRBridge:
    ...
    def _populate(self):
        ...
        try:
            ...
            logger.warning("Failed to fetch NBI info for %s: %s",
                           self.bridge_id, e)

        try:
            self._fetch_controlling_rating()
        except Exception as e:
            logger.warning("Failed to fetch controlling rating for %s: %s",
                           self.bridge_id, e)

        try:
            self._fetch_member_capacities()
        except Exception as e:
            logger.warning("Failed to fetch member capacities for %s: %s",
                           self.bridge_id, e)

        try:
            self._fetch_vehicle_loads()
        except Exception as e:
            logger.warning("Failed to fetch vehicle loads for %s: %s",
                           self.bridge_id, e)

    def _fetch_nbi_info(self):
        """Fetch basic bridge identification from ABW_BRIDGE."""
        query = self._text("""
                           SELECT AGENCY_CODE,
                                  BRIDGE_ID,
                                  STRUCT_NUM,
                                  ...
                                  COUNTY_PARAM_ID,
                                  OWNER_PARAM_ID
                           FROM BRIDGEWARE.ABW_BRIDGE
                           WHERE TRIM(AGENCY_CODE) = :sfn
                           """)
        ...
        if row:
            self.nbi_info = {
                "brkey": row[0],  # AGENCY_CODE acts as the BRKEY
                "bridge_id": row[1],
                "struct_num": row[2],
                "name": row[3],
                ...
                "facility": row[5],
                "location": row[6],
                "district_id": row[7],  # Note: These are now Parameter IDs, not text!
                "county_id": row[8],
                "owner_id": row[9],
                "year_built": row[10],
                # The legacy rating loads and posting statuses aren't in this table
                "rating_date": None,
                "operating_load": None,
                "operating_type": None,
                "inventory_load": None,
    ...
    def _fetch_controlling_rating(self):
        query = self._text("""
                           ...
                           WHERE TRIM(b.AGENCY_CODE) = :bridge_id
                             AND e.UP_TO_DATE_IND = 'T'
    ...
    def _fetch_structural_data(self):
        """Fetch bridge hierarchy, spans, and member names."""
        # 1. Fetch Member Names and IDs
        member_query = self._text("""
                                  SELECT SUPER_STRUCT_MBR_ID, NAME, DESCR
                                  FROM BRIDGEWARE.ABW_SUPER_STRUCT_MBR
                                  WHERE BRIDGE_ID =
                                        (SELECT BRIDGE_ID FROM BRIDGEWARE.ABW_BRIDGE WHERE TRIM(AGENCY_CODE) = :sfn)
                                  """)

        # 2. Fetch Span Lengths tied to those members
        span_query = self._text("""
                                SELECT SUPER_STRUCT_MBR_ID, SPAN_NUM, LENGTH
                                FROM BRIDGEWARE.ABW_SUPER_STRUCT_MBR_SPAN
                                WHERE BRIDGE_ID =
                                      (SELECT BRIDGE_ID FROM BRIDGEWARE.ABW_BRIDGE WHERE TRIM(AGENCY_CODE) = :sfn)
                                ORDER BY SUPER_STRUCT_MBR_ID, SPAN_NUM
                                """)

        with self.engine.connect() as conn:
            members = conn.execute(member_query, {"sfn": self.bridge_id}).fetchall()
            spans = conn.execute(span_query, {"sfn": self.bridge_id}).fetchall()

        # Organize into a usable structure
        struct_map = {m[0]: {"name": m[1], "descr": m[2], "spans": []} for m in members}

        for s_mbr_id, s_num, s_len in spans:
            if s_mbr_id in struct_map:
                struct_map[s_mbr_id]["spans"].append({
                    "span_no": s_num,
                    "length": s_len
                })

        self.structural_data = list(struct_map.values())

    def fetch_design_info(self):
        """
        Fetches design info using the confirmed common key: SPNG_MBR_DEF_ID.
        This handles both Rolled Stringers and Plate Girders.
        """
        query = self._text("""
                           SELECT m.NAME                                              AS member_name,
                                  -- Coalesce looks at ISHAPE first (rolled), then XSECTION (plate)
                                  COALESCE(ishape.DEPTH, xsec.WEB_DEPTH)              AS depth,
                                  COALESCE(ishape.WEB_THICK_TW, xsec.WEB_THICK)       AS web_thickness,
                                  COALESCE(ishape.FLNG_WIDTH_BF, xsec.FLNG_WIDTH_TOP) AS flange_width,
                                  xsec.FLNG_WIDTH_BOT                                 AS flange_width_bot,
                                  xsec.HAUNCH_THICK                                   AS haunch_thickness,
                                  xsec.DECK_ACTUAL_THICK                              AS deck_thickness,
                                  xsec.DECK_TRIBUTARY_WIDTH                           AS trib_width,
                                  xsec.DECK_COMPOSITE_IND                             AS composite
                           FROM BRIDGEWARE.ABW_SUPER_STRUCT_MBR m
                                    JOIN BRIDGEWARE.ABW_SUPER_STRUCT_SPNG_MBR_ALT alt
                                         ON m.BRIDGE_ID = alt.BRIDGE_ID
                                             AND m.SUPER_STRUCT_MBR_ID = alt.SUPER_STRUCT_MBR_ID
                               -- Link to Rolled Shapes using the shared SPNG_MBR_DEF_ID
                                    LEFT JOIN BRIDGEWARE.ABW_STL_ISHAPE ishape
                                              ON alt.BRIDGE_ID = ishape.BRIDGE_ID
                                                  AND alt.SPNG_MBR_DEF_ID = ishape.STL_SHAPE_ID
                               -- Link to Plate/General Section Info using the same shared key
                                    LEFT JOIN BRIDGEWARE.ABW_STL_XSECTION xsec
                                              ON alt.BRIDGE_ID = xsec.BRIDGE_ID
                                                  AND alt.SPNG_MBR_DEF_ID = xsec.SPNG_MBR_DEF_ID
                           WHERE m.BRIDGE_ID =
                                 (SELECT BRIDGE_ID FROM BRIDGEWARE.ABW_BRIDGE WHERE TRIM(AGENCY_CODE) = :sfn)
                           """)

        with self.engine.connect() as conn:
            results = conn.execute(query, {"sfn": self.bridge_id}).fetchall()

        self.design_info = [dict(r._mapping) for r in results]

    def display_design_summary(self):
        """Prints a scannable summary of the bridge's physical design."""
        if not hasattr(self, 'design_info'):
            self.fetch_design_info()

        print(f"\nDesign Summary for {self.bridge_id}:")
        print(f"{'Member':<20} | {'Depth (mm)':<10} | {'Web (mm)':<8} | {'Top Flg':<8} | {'Comp?'}")
        print("-" * 65)

        for d in self.design_info:
            # Handle None types for numeric formatting
            depth = f"{d['depth']:.2f}" if d['depth'] is not None else "N/A"
            web = f"{d['web_thickness']:.2f}" if d['web_thickness'] is not None else "N/A"
            flg = f"{d['flange_width']:.2f}" if d['flange_width'] is not None else "N/A"

            comp_status = "Yes" if d['composite'] == 'T' else "No"

            print(f"{d['member_name']:<20} | {depth:<10} | {web:<8} | {flg:<8} | {comp_status}")

    # Update your __str__ to include this:
    def __repr__(self):
        # ... (keep existing rating print logic) ...
        res = [f"Structure Info for {self.bridge_id}:"]
        for m in self.structural_data:
            span_str = ", ".join([f"S{s['span_no']}: {s['length']}" for s in m['spans']])
            res.append(f"  - {m['name']} ({m['descr'] or 'No Descr'}): {span_str}")
        return "\n".join(res)
...
class AssetwiseRawBridge:
    ...
    def _resolve_asset(self):
        ...
        try:
            ...
            logger.warning("Failed to resolve asset for SFN %s: %s", self.sfn, e)
    ...
    def _fetch_current_values(self):
        ...
        try:
            ...
            logger.warning("Failed to fetch current values for as_id=%s: %s",
                           self.as_id, e)
